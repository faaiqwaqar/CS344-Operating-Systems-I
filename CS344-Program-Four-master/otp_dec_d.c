/*otp_dec_d.c*/
#include "universal.h"
/*decrypt data, takes the key string and the string and converys it into non-scryptic*/
void decryptData(struct otpEnc *oe){
  int i;
   ...
   for(i = 0; i < oe->fileBufferLength; i++){
     ...
     /*compute an index of the noncryptic message using mod and place into the file buffer*/
     localBuffer3 = localBuffer1 - localBuffer2;
     if(localBuffer3 < 0){
       localBuffer3 += 27;
     }
  ...
  while(1){
    ...
    switch(oe->pid){
      ...
      case 0:
        ...
        oe->charsRead = send(oe->establishedConnectionFD, "d", 39, 0);
        if (oe->charsRead < 0){
        ...
        /*decrypt and send the data back*/
        decryptData(oe);
